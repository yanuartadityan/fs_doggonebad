...
async def create():
    ...
    # method 2
    await Group.objects.create(group_name="Family-Doggonebad", is_max_reached=False)

    await Group.objects.create(group_name="Family-Catto", is_max_reached=False)

    await Group.objects.create(group_name="SMA34 Commuting Group", is_max_reached=False)

    a_group = await Group.objects.create(group_name="SMA8_JakSel_Team")
